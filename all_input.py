...
def build_combined_input_with_compounds(mechanisms, nn_mechanisms=None, output_dir="Kinetics_All", output_file="input.mkm"):
    compound_header = [
        "&compounds",
        "N2;   0;  0.9",
        "CO;   0;  0.08",
...
""".strip()
    ...
    for mech in mechanisms:
        file_path = os.path.join(f"Kinetics_{mech}", "input.mkm")
    ...
    # Nur für nn-Mechanismen
    if nn_mechanisms:
        for mech in nn_mechanisms:
            file_path = os.path.join("nn", f"Kinetics_{mech}", "input.mkm")
            if os.path.isfile(file_path):
                _, compound_blocks = extract_compounds_block(file_path)
                for label, block in compound_blocks.items():
                    if block not in all_compound_blocks.values():
...
if __name__ == "__main__":
    ...
    nn_mechanisms = ["Mvknn"]
    build_combined_input_with_compounds(mechanisms, nn_mechanisms)
